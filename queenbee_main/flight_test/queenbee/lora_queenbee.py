...
class Lora:
    ...
    async def cycle_send_case_position(self, drone:Bee, a, b):
        await asyncio.sleep(5)
        while True:
            if self.is_on:
                case_position = f"lat:{str(a)[0:9]},lng:{str(b)[0:9]}"
    ...
    #     while case_.ALMOSTFINISHCASE == False or case_.FINISHCASE == False:
    ...
    #         time.sleep(10)  # this must be a little long because lora cannot receive message while sending
    #     return
    
    # def do_cycle_send_progress(self,case_:Case):
    #     self.cycle_send_progress(case_)            
    
    async def cycle_send_progress_test(self):
        while True:
            progress = f"progress:apple"
            print(f"send progress{progress}")
                # await self.write(lat)
                # await self.write(lng)
                # await self.write(alt)
    ...
    # def do_cycle_send_progress_test(self):
    #     self.cycle_send_progress_test()      

    def do_cycle_send_progress_test(self):
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        loop.run_until_complete(self.cycle_send_progress_test())
        loop.close()  
