...
if __name__ == "__main__":    
    ...
    lonlst = []
    altlst = []
    # df = pd.read_csv(r"..\\log\\2023-07-02_logger\\010.csv")
    df = pd.read_csv(r"..\\log\\2023-06-29_logger\\007.csv")
    for i in range(len(df)):
        if df.iloc[i]["Message"] == 'MANUAL' or df.iloc[i]["Message"] == 'HOLD' or df.iloc[i]["Message"] == 'LAND' or df.iloc[i]["Message"] == 'TAKEOFF':
            latlst.append(df.iloc[i]["Latitude"])
            lonlst.append(df.iloc[i]["Longitude"])
    ...
    latlst = np.array(latlst)
    latlst -= latlst[0]
    latlst *=111100
    
    lonlst = np.array(lonlst)
    lonlst -= lonlst[0]
    lonlst *= 91015
    ...
    ax.scatter(latlst[0], lonlst[0],altlst[0], color = 'r', label = 'start')
    ax.scatter(latlst[-1], lonlst[-1], altlst[-1], color = 'g', label = 'goal')
    ax.set_xlim(min(min(latlst), min(lonlst)), max(max(latlst), max(lonlst)))
    ax.set_ylim(min(min(latlst), min(lonlst)), max(max(latlst), max(lonlst)))
    ax.legend()
    ax.view_init(45,45)
    plt.show()
    ...
    # ]
    # folium.PolyLine(locations=sq).add_to(m)    
    # m.save(r"..\\log\\2023-07-02_logger\\010.html")
