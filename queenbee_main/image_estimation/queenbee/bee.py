...
###loop#########################################################################################
    ...
    async def Loop_pressure(self) ->None:
        while True:
            logger_info.info(self.Pressure)
            await asyncio.sleep(2)
    ...
    async def Loop_goto_location(self, latitude_target,
        ...
        await self.Goto_location(latitude_target,
                                  longitude_target,
                                  altitude_target,0)
        while abs(altitude_target - self.Absolute_altitude_m) > 0.3 or abs(latitude_target - self.Latitude_deg) > 1.0e-6 or abs(longitude_target - self.Longitude_deg) > 1.0e-6:
            await asyncio.sleep(0.01)
